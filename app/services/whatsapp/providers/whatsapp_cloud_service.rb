class Whatsapp::Providers::WhatsappCloudService < Whatsapp::Providers::BaseService
  ...
  def send_text_message(phone_number, message)
    response = HTTParty.post(
      ...
      body: {
        ...
        to: phone_number,
        text: { body: format_message_content(message) },
        type: 'text'
      }.to_json
    )
  ...
  def send_attachment_message(phone_number, message)
    ...
    type_content['caption'] = format_message_content(message) unless %w[audio sticker].include?(type)
  ...
  def format_message_content(message)
    return message.outgoing_content unless should_add_agent_prefix?(message)

    agent_name = message.sender&.available_name || message.sender&.name
    return message.outgoing_content unless agent_name
  ...
  def should_add_agent_prefix?(message)
    ...
    return false unless [4, 5].include?(message.conversation.account_id)
